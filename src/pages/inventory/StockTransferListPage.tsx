...
export function StockTransferListPage() {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('ยืนยันการลบใบโอนย้ายสต็อกนี้?')) return
    setDeleteId(id)
    try {
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-gray-400">ไม่พบข้อมูลโอนย้ายสต็อก</td>
                </tr>
              ) : (
                transfers.map((transfer) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3 font-mono text-xs text-gray-600">{transfer.code}</td>
                      ...
                      <td className="px-4 py-3 text-gray-600">{transfer.items?.length ?? 0} รายการ</td>
